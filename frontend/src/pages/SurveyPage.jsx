  ...
  return (
    <div className="page">
      ...
      {(mode !== 'edit-pending' || record) && (
        <SurveyForm initialValues={isEditMode ? record || undefined : { surveyorId: session?.username || '' }} lockedFields={mode === 'create' ? ['surveyorId'] : mode === 'edit' && session?.role !== 'ADMIN' ? ['surveyId'] : []} onSubmit={save} loading={loading} submitLabel={isEditMode ? 'Update survey' : 'Submit survey'} unlockAllSections={isEditMode} />
      )}
    </div>
  );
}
